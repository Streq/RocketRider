...
GameObjectDefinition * def_from_xml(rapidxml::xml_node<>* node)
{
	...
	switch (type) {
		...
		case ObjectType::Player: {
			...
			if (fuel_node) { play->fuel = std::stof(fuel_node->value()); }
			auto* max_fuel_node = node->first_node("max_fuel");
			if (max_fuel_node) { play->max_fuel = std::stof(max_fuel_node->value()); }
			auto* rope_length_node = node->first_node("rope_length");
			if (rope_length_node) { play->rope_length = std::stof(rope_length_node->value()); }
			auto* max_speed_node = node->first_node("max_speed");
			if (max_speed_node) { play->max_speed = std::stof(max_speed_node->value()); }
			auto* angular_acceleration_node = node->first_node("angular_acceleration");
			if (angular_acceleration_node) { play->angular_acceleration = std::stof(angular_acceleration_node->value()); }
			auto* explosion_impulse_node = node->first_node("explosion_impulse");
			if (explosion_impulse_node) { play->explosion_impulse = std::stof(explosion_impulse_node->value()); }
...
sf::Color parse_color_from_xml(rapidxml::xml_node<>* node)
{
	...
	const unsigned long alpha_val = alpha ? std::stoul(alpha->value(), NULL, 10) : 255;
	/*
